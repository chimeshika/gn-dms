...
class OfficerImportService
{
     ...
     */
    public function import(string $filePath, ?int $batchDsDivisionId = null, string $disk = 'local', ?User $actor = null): array
    {
        ...
        foreach ($rawRows as $rawRow) {
            ...
            $existing = Officer::where('nic_no', $nic)->first();
            if ($actor && ! $actor->isMainAdmin() && $existing) {
                AccessScope::officer($actor, $existing->id);
            }
            ...
            if ($actor && ! $actor->isMainAdmin() && ! $existing) {
                $officer->update([
                    'current_district_id' => $actor->district_id,
                    'current_ds_division_id' => $actor->isDivisionalAdmin() ? $actor->ds_division_id : $batchDsDivisionId,
                ]);
            }
            if (! $officer->user_id) {
    ...
    protected function buildHeaderMap(mixed $firstRow): array
    {
        ...
        foreach (array_keys($firstRowArray) as $rawHeader) {
            ...
            foreach (self::COLUMN_ALIASES as $col => $aliases) {
                if ($normalised === str_replace('_', ' ', $col) || in_array($normalised, $aliases, true)) {
                    $canonical = $col;
                    break;
                }
            }
