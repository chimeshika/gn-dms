...
class SignatoryService
{
    ...
    public function findForCategory(SignatoryCategory $category, ?int $dsDivisionId = null, ?int $districtId = null): ?Signatory
    {
        return Signatory::query()
            ->where('is_active', true)
            ->where('category', $category->value)
            ->when($dsDivisionId, fn ($q) => $q->where('ds_division_id', $dsDivisionId)->orWhereNull('ds_division_id'))
            ->when($districtId, fn ($q) => $q->where('district_id', $districtId)->orWhereNull('district_id'))
            ->orderBy('id')
            ->first();
    }
